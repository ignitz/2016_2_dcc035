...
/*
 ...
 obj:  x1 +2x2 +  3x3  + x4
 Subject To
 c1: 0   <= -1 x1 + 1 x2  + 1 x3 + 10 x4  <= 20    A = | -1   1   1   10|
 c2: 0   <=  1 x1 - 3 x2  + 1 x3          <= 30        |  1  -3   1    0|
 c3:         1 x2        - 3.5 x4         = 0          |  0   1   0 -3.5|
 Bounds
 0 <= x1 <= 40
 2 <= x4 <= 3
 General
...
int main(int argc, char *argv[]) {
    ...
    // ------------------------------------------
    // define a funcao objetivo(maximizacao/minimizacao)
    glp_set_obj_dir(mip, GLP_MAX);

    // ------------------------------------------
    // addiciona restricoes
    glp_add_rows(mip, 3);  // 3 restricoes
    ...
    glp_set_col_kind(mip, 4, GLP_;

    // define a matriz A.
    int ia[1+9] /* i*/, ja[1+9] /* j*/;
    double ar[1+9];
    ...
    // adiciona a matriz A no modelo.
    glp_load_matrix(mip, 9, ia, ja, ar);

    // define os parametros do solver.
    glp_iocp parm;
    glp_init_iocp(&parm);
